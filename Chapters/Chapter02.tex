...
%************************************************
The LHC produces over 40 millions collisions proton-proton per second in ATLAS or CMS detectors. The \emph{raw} data is made up of single-byte signals from roughly 100 millions acquisition channels, meaning \emph{4 PB/s of raw data}. The actual amount of data collected for each event, including pile-up, is around 1 MB (1 Megabyte). This means that we are reaching 40 TB/s of data--far too much for any conventional data acquisition system to handle.
...
\begin{outline}
    \1 7 PB of RAW data (due to backup copies) and 3.5 PB of reconstructed data, 14 PB or RAW Monte Carlo and 7 PB of reconstructed simulation, for a total of 40 PB in a year;
...
As the luminosity increases and the energy of the LHC reaches new and unprecedented energies, all these figures are expected to increase.
