...
which has the nice property of presenting a smooth transition between the two saturating extremes of the Perceptron. Nowadays, the \emph{Rectified Linear Unit} (ReLU) function has demonstrated better capabilities, especially in large and complex networks:
...
despite not being analytically differentiable in $0$ (it is standard practice to set the derivative in the point to $0$).

\begin{figure}
    \centering
    \includegraphics[scale=0.4]{gfx/ch2/sigmvsrelu.png}
    \caption[Activation functions]{The sigmoid activation function compared to the ReLU.}
    \label{fig:actfunc}
\end{figure}
