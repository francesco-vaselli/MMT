...
The LHC tunnel is not exactly circular in shape but it is formed by 8 straight sections and 8 circular portions. Each arc has an internal radius of 3.7 km and contains
...
possibility to cross check each other’s studies; however, they also present unique design choices setting them apart from each other. ALICE (A Large Ion Collider Experiment) studies a phase of matter called quark-gluon plasma that is formed in
...
accelerators. Then, they are injected in the LHC with opposite directions in
two separate beam pipes with an energy of 450 GeV. The particles are accelerated
inside LHC by \emph{radio-frequency cavities} from 450 GeV to a design-energy of 7 TeV (reached 6,8 TeV maximum), therefore
...
The collision rate of an accelerator is measured by its \emph{luminosity}. It's a measurement of the number of collisions that can be produced in a detector per cm$^{2}$ (of process cross-section) and per second. The bigger is the value of L, the bigger is the number of collisions, making L a key parameter in the design of an accelerator.

Semi-qualitatively, we can define L from a series of key quantities. The \emph{transverse emittance} $\epsilon$ is a beam quality concept reflecting the process of bunch preparation (the injector chain), extending all the way back to the source for hadrons.
...
The \emph{amplitude function} $\beta$  is determined by the accelerator magnet configuration, in terms of $\sigma \approx 16 \; \mu$m, (cross-sectional size of the bunch)  and  the transverse emittance:

\[\beta = \frac{\pi\sigma^2}{\epsilon}\]

So, $\beta$ is roughly the width of the beam squared divided by the emittance. If $\beta$ is low the beam is narrower, \emph{squeezed}. 
Of particular significance is the value of the amplitude function at the interaction point, $\beta^*$, expressed in units of length. Clearly one wants  to be as small as possible; how small depends on the capability of the hardware to make a near-focus at the interaction point.
Nominal values for these quantities are $\beta^* = 0.55$ m and $\epsilon = 3,75 \; \mu$m rad.

If we define $N \approx (1.15\cdot10^{11})$ as the number of protons per bunch, $t$ = 25 ns as the time between bunches, meaning a \emph{bunches crossing} frequency $f \approx 40\cdot 10^6$ Hz we obtain L as:
...
The final expected rate of events $R$ for a process with cross section $\sigma$ is then:
...
be searched, but then we also suffer from a worse event reconstruction.
...
5 wheel in the barrel and three disks in each endcap. The strong magnetic flux ensures enough bending power to measure the momentum of the highly energetic muons that
cross the detector.
...
What follows is a brief description of the major subdetectors. As the technical details of each subdetector are extremely subtle, they have already been studied in numerous articles and are not the focus of our study, we will not discuss them in the present work. The interested reader can found a detailed reference at \cite{Collaboration_2008}. Several upgrades have already been performed during the operational history of the detector, and more are to come in preparation for the \emph{High Luminosity} phase of the LHC.
...
tracker, they are required to completely absorb the particles in the shower for optimal energy measurements. They are therefore required to have a large
number of radiation lengths $X_0$ for the ECAL and of interaction lengths $\lambda_I$ for the HCAL, namely, to be \emph{heavy}.
...
\begin{outline}
    ...
    where a $\approx$ 0.12 GeV is the noise term due the electronics and pile-up, independently of the energy, b $\approx$ 2.8$\% \; \sqrt{\text{GeV}}$ is
...
good time resolution. The RPCs are instead mainly used for their good time resolution.


\paragraph{Trigger}

The 40 MHz rate of proton-proton collisions and the pile-up make it impossible to process
...
\emph{High Level Trigger} (HLT) further decreases the event rate from about 100 kHz to about 1 kHz for data storage. The HLT is implemented by a computer farm composed of more than 30,000 CPUs
running software modules similar to the ones used for the offline reconstruction. 

\paragraph{Offline reconstruction}\label{sec:offreco}
...
Particularly complex and important algorithms include:
...
\begin{outline}
    ...
    The tracking efficiency saturates at $\approx$ 90$\%$ for charged hadrons due to nuclear interactions in the tracker material, while it is known to be close to 100$\%$ for muons;
    
    \1 Once each subdetector has reconstructed its information, everything is passed to the \emph{particle flow} algorithm which is responsible for transforming the pre-processed information from each sub-detector into a list of final state particles produced in the event and their properties. It takes as inputs tracks (also from muon chambers) and caloritmeters clusters, and defines a PF \emph{block} as the building unit of a physical object, defined as a series of links between single hits. Muons, electrons and isolated photons, as well as neutral and charged hadrons are all reconstructed at this step and are called \emph{PFcandidates}. Jets are obtained by clustering PFcandidates, and we also employ specific \emph{clustering algorithms} (see \cite{Cacciari_2012}). The \emph{transverse missing energy} (MET) is obtained as:
    \[\text{MET} = -\sum_{i \in \text{PFcands}} \Vec{p_T^i}\]
     The \emph{isolation} of the various lepton and photon objects is also calculated at this step, checking for the presence of hadrons and photons in a cone centered around our target and computing their momentum; 

\end{outline}

\paragraph{Luminosity and Pile-up}

 The LHC design luminosity of 10$^{34}$ cm$^{-2}$ s$^{-1}$ has been achieved and exceeded in the last three years of data taking, and a new data taking has just started with a goal of $2\cdot 10^{34}$ cm$^{-2}$ s$^{-1}$. Integrated luminosity recorded by CMS and employable for physics analyses are 35.9 fb$^{-1}$ for 2016, 41.53 fb$^{-1}$ for 2017, and 60.0 fb$^{-1}$ for 2018.
...
Being a general-purpose experiment, CMS is capable of performing a wide range of interesting and insightful measurements. Aside from the discovery of the Higgs boson and the study of its couplings, which will be discussed below in greater detail, here we briefly mention the other directions of CMS research:

\begin{outline}
    \1 The \emph{SM physics} programme includes all the studies of the LHC data with the goals of measuring fundamental parameters of the Standard Model of particle physics. The cross-sections of a large variety of processes happening during the beam collisions are determined with the best precision possible and compared to the most up-to-date theoretical calculations, from the most common to the rarest. Fundamental parameters of the theory such as masses of elementary particles and gauge couplings are measured to verify the effectiveness of the predictions, and any deviations of data from the Standard Model are quantified. The various \emph{couplings} expected between particles are investigated in detail. As an example, the \emph{Vector Boson Scattering} is a series of processes such as $W^{+}W^{-}\rightarrow W^{+}W^{-}$ and $ZZ \rightarrow ZZ$, however, it can admit higher-order couplings to the Higgs boson as $ZZ \rightarrow H^{\pm} \rightarrow ZZ$ which are to be investigated to compare their productions to what is expected from theory;
    ...
    \1 Finally, the \emph{heavy-ions} group manages the analysis of data collected during heavy-ions runs.
    
\end{outline}

\subsection{The Higgs discovery and searches}

The main driver for the construction of the LHC and the creation of the CMS experiment is certainly the Higgs boson and the detailed study of its properties. 

The Higgs is a \emph{scalar boson}, whose presence in the theory is needed in order to explain the masses of the \emph{gauge bosons} (as well as the fundamental fermions) through the \emph{Higgs mechanisms}: a spontaneous symmetry breaking of the U(1)$\times$SU(2) local gauge symmetry of the electroweak sector. The mass of the Higgs particle is a \emph{free parameter} of the theory. The Higgs boson is also expected to  interact with the entire fermion sector trough \emph{Yukawa}-like vertices, with a coupling proportional to the fermion mass.

The Higgs boson, which is consistent with the SM predictions to the current experimental data, was discovered in 2012 at the LHC (see \cite{Aad_2012} and \cite{Chatrchyan_2012}). The mass of the new particle, approximately 125 GeV, sets the boundaries of today well-know Higgs phenomenology.
