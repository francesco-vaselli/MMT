...
\begin{outline}
    ...
    \1 Once each subdetector has reconstructed its information, everything is passed to the \emph{particle flow} algorithm which is responsible for transforming the pre-processed information from each sub-detector into a list of final state particles produced in the event and their properties. It takes as inputs tracks (also from muon chambers) and caloritmeters clusters, and defines a PF \emph{block} as the building unit of a physical object, defined as a series of links between single hits. Muons, electrons and isolated photons, as well as neutral and charged hadrons are all reconstructed at this step and are called \emph{PFcandidates}. Jets are obtained by clustering PFcandidates and the \emph{transverse missing energy} (MET) is obtained as:
    \[\text{MET} = -\sum_{i in PFcands} p_T^i\]
    For identifying jets, we also employ specific \emph{clustering algorithms}. The \emph{isolation} of the various lepton and photon objects is also calculated at this step, checking for the presence of hadrons and photons in a cone centered around our target and computing their momentum; 
